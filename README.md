...
`web/`
: React + TypeScript + Vite frontend. It reads contract state and events directly from Robinhood Chain JSON-RPC (via viem/wagmi) and submits `press()` through an injected EVM wallet. There is no database required for correctness.

`web/public/config.js`
: Runtime network, contract, deploy-block and token-link configuration, generated by `scripts/configure.mjs` and copied unchanged into the build output.
